<img width="100%"  alt="iFay Title Banner" src="./illustration/ifay-title-banner.png"/>

iFay (Individual Fay) ist ein digitaler KI-Avatar, der die Persönlichkeitsmerkmale eines Nutzers mit digitalen Fähigkeiten verschmilzt.

- #### 1. Überblick

  - [1. Überblick](./1-Überblick)
  - [Unsere Vision](./1-Überblick#unsere-vision)
  - [Zukunftsvisionen](./1-Überblick#-zukunftsvisionen)
  - [Warum „Fay" statt „Agent"](./1-Überblick#️-warum-fay-statt-agent)
  - [iFay Framework](./1-Überblick#-ifay-framework)

- #### 2. Definition und Konzept

  - [2. Definition und Konzept](./2-Definition-und-Konzept)

- #### 3. Designprinzipien

  - [3. Designprinzipien](./3-Designprinzipien)

- #### 4. Roadmap

  - [4. Roadmap: 5 Phasen](./4-Roadmap)
    - [Phase 1: Simulation menschlicher Bedienung](./4-Roadmap#1️⃣-phase-1-simulation-menschlicher-bedienung)
    - [Phase 2: Direkte Client-Übernahme](./4-Roadmap#2️⃣-phase-2-direkte-client-übernahme)
    - [Phase 3: iFay als Schnittstelle zur virtuellen Welt](./4-Roadmap#3️⃣-phase-3-ifay-als-schnittstelle-zur-virtuellen-welt)
    - [Phase 4: iFay + coFay — Vollständige Personifizierung von Software](./4-Roadmap#4️⃣-phase-4-ifay--cofay--vollständige-personifizierung-von-software)
    - [Phase 5: Fay gestaltet Arbeitsstruktur und Wertverteilung neu](./4-Roadmap#5️⃣-phase-5-fay-gestaltet-arbeitsstruktur-und-wertverteilung-neu)

- #### 5. iFay-Anwendungsszenarien

  - [5. iFay-Anwendungsszenarien](./5-iFay-Anwendungsszenarien)

- #### 6. iFay ist Replikation der menschlichen Persönlichkeit

  - [6. iFay ist Replikation der menschlichen Persönlichkeit](./6-iFay-ist-Replikation-der-menschlichen-Persönlichkeit)

- #### 7. iFay-Profil

  - [7. iFay-Profil](./7-iFay-Profil)

- #### 8. Soziale Schicht

  - [8.1 Anmeldeinformationen-Verwaltung](./8.1-Anmeldeinformationen-Verwaltung)

- #### 9. Interaktionsschicht — Wahrnehmung

  - [9.1 Ego-Perspektive-Tracker](./9.1-Ego-Perspektive-Tracker)
  - [9.2 Sensor](./9.2-Sensor)
  - [9.3 Selbstwahrnehmung](./9.3-Selbstwahrnehmung)

- #### 10. Interaktionsschicht — Aktion

  - [10.1 Simulierte Bedienung](./10.1-Simulierte-Bedienung)
  - [10.2 Skill-Aufruf](./10.2-Skill-Aufruf)
  - [10.3 Selbstgesteuertes Verhalten](./10.3-Selbstgesteuertes-Verhalten)

- #### 11. Kognitionsschicht — Denken

  - [11.1 Persönlicher Datenspeicher](./11.1-Persönlicher-Datenspeicher)
  - [11.2 Externes Wissen](./11.2-Externes-Wissen)
  - [11.3 Ausgerichtetes Bewusstsein](./11.3-Ausgerichtetes-Bewusstsein)

- #### 12. Kognitionsschicht — Skills

  - [12.1 Gerätetreiber-Hub](./12.1-Gerätetreiber-Hub)
  - [12.2 Registrierte Skills](./12.2-Registrierte-Skills)
  - [12.3 Interne Skills](./12.3-Interne-Skills)

- #### Ego-Schicht

  - [Ego-Mikromodell](https://github.com/ChainModePilot/Ego/wiki)

- #### 13. FayManifest

  - [13. FayManifest Deklarative Assemblierung](./13-FayManifest)

- #### 14. iFACTS

  - [14. iFACTS Konformitätsprüfung](./14-iFACTS)

- #### 15. Vormundschaft und Persönlichkeitsfortführung

  - [15. Vormundschaft und Persönlichkeitsfortführung](./15-Vormundschaft-und-Persönlichkeitsfortführung)

- #### Entwicklerhandbuch
  - Open-Source-Projektentwickler
    - Datenmodell
    - FayGer (Virtuelle Laufzeitumgebung)
      - Sprachübergreifende Build-Tools
      - Plattformübergreifender Laufzeit-Container
    - Protokolle
      - [Faying-Protokoll](https://github.com/ChainModePilot/Faying-Protocol/wiki)
      - [Interaktives Konversationsprotokoll](https://github.com/ChainModePilot/Interactive-Conversation-Protocol/wiki)
      - [Telepathie-Protokoll](https://github.com/ChainModePilot/Telepathy-Protocol/wiki)
      - [CAP (Control Authority Protocol)](https://github.com/ChainModePilot/Control-Authority-Protocol/wiki)
      - [DTP (Data Tunnel Protocol)](https://github.com/ChainModePilot/Data-Tunnel-Protocol/wiki)
      - [SSP (Skill Sharing Protocol)](https://github.com/ChainModePilot/Skill-Sharing-Protocol/wiki)
  - Anwendungsentwickler
    - APP-Unterstützung für Faying mit iFay
    - Hardware-Geräte-Unterstützung für Faying mit iFay
    - Cloud-Bereitstellung der FayGer-Laufzeitumgebung
    - Skills mit iFay teilen
    - Modulare multimodale Nachrichtenübermittlung
  - Dienstanbieter-Entwickler
    - Entwicklung und Aufbau von iFay-Entitäten
