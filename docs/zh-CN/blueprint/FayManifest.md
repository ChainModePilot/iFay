...
| 模式 | 名称 | 说明 |
|------|------|------|
| `command` | **指令控制** | 宿主明确驱动每一个动作。iFay 不会自主行动，每次操作都需要宿主的明确指令。适用于高风险场景或初始信任建立阶段。 |
| `ego` | **Ego 控制** | Ego 模型在约束范围内自主决策。iFay 可以根据宿主的个性和偏好自主判断和行动，但行为边界由 Ego 模型约束。适用于日常助手场景。 |
...
- [iFACTS](./iFACTS) — 合规性验证
- [路线图](./Roadmap:-5-steps) — 阶段配置
- [设备驱动中枢](./Device-Driver-Hub) — 驱动配置
