...
1. 凭证委托：人类用户必须能够通过可控且可审计的委托机制，安全地授权 iFay 使用其[凭证](./Definition-and-Concept#通用概念澄清)（账号、密码、证书、访问权限、合约等）。
...
- _**激活状态**_：为确保 iFay 永远不会在人类宿主不知情或无意图的情况下运行，我们定义了严格的激活规则。任何 iFay 都不应在没有明确意图的情况下自主行动。这由开源的 [Faying 协议](https://github.com/ChainModePilot/Faying-Protocol/wiki)管理，该协议规定了自然人与 iFay 如何安全配对，以及在什么条件下 iFay 被授权以激活状态运行。
...
注意：最初，所有这些票据都来自宿主用户。为了更高的安全性和更便捷的管理，宿主的所有凭证都将被交换为与原始凭证对应的副本。iFay 使用此副本进行登录和认证。

当然，我们并不认为 iFay 不能拥有自己的凭证而宿主不能使用它们。因此，每种凭证都会标明其原始所有者是宿主还是 iFay 本身。
...
通过将其感官感知与人类宿主对齐，iFay 可以做出与人类意图密切匹配的判断和决策。
...
与[第一阶段](./Roadmap:-5-steps#1️⃣-第一阶段模拟人类操作)相比，iFay 新增了五个内部模块，首先是：
...
宿主只需要与自己的 iFay 通信，iFay 然后根据宿主的意图调用所需的服务。

由于宿主查看信息的界面由 iFay 组合和渲染，它实际上扮演了浏览器的角色。

由于引入 iFay 的核心动机是使其成为超越宿主的智能延伸，其增强能力通过注册技能实现。
在思维领域，我们引入以下模块：
...
通过此技能获取的知识和信息与 iFay 的个人数据一起管理，最终实现超越宿主自身能力的智能。
...
与机器不同，iFay 本身不能拥有真正的情感。但通过观察其宿主和周围上下文，它可以从感知中推断感受。
这是构建 iFay 自我感知的核心策略。
...
### 技能 → 内部技能
我们引入内部技能模块有三个主要目的：
- 建立与宿主个性对齐的习惯，包括对外部技能的潜在限制或治理。
- 提供内省机制，确保外部知识永远不与宿主意图冲突。
- 嵌入固定的宿主特定能力，如专业技能和专业知识。
